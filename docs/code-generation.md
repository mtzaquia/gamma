...
All `*.theme.json` files in one target are treated as variants of one theme family. Their existing JSON format does not change. The plugin generates one token file and checks that every variant defines the same alias contract, failing with both paths when aliases drift.
